...
const AutocompleteWithName: React.FC<AutocompleteProps> = ({ data, value, name, onChange, onNoMatchClick, onInput, isLoading = false }) => {
  ...
  const handleItemClick = (item: string) => {
    onChange({ target: { name, value: item } } as React.ChangeEvent<HTMLInputElement>);
    setIsOpen(false);
  };
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && filteredData.length === 0 && value && value?.trim() !== '' && (
        <div className="mt-1">
          <button
            type="button"
